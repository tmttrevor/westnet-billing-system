...
export class PaymentOrchestratorService {
  ...
  async startStkPush(input:{tenantId:string;customerId:string;planId:string;phone:string}) {
    ...
    const payment=await this.ledger.createPending({tenantId:input.tenantId,reference,phone:input.phone,amount:Number(plan.price)});
    try {
